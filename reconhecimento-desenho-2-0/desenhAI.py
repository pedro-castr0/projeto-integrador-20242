...
class DrawPredict:
    def __init__(self):
        self.class_names = ['airplane', 'ant', 'apple', 'axe', 'basketball', 'bee', 'bicycle', 'bird', 'cat', 'circle', 'clock', 'dog', 'donut', 'eiffel tower', 'flower', 'house', 'smiley face', 'snail', 'square', 'tree']
    ...
    def predict_proba(self):
        ...
        with torch.no_grad():
            ...
            probs = torch.softmax(logits, dim=1).cpu().numpy().flatten()  # Mover para CPU antes de usar .numpy()
    ...
    def update_prediction_loop(self):
        try:
            ...
            self.prob_label.configure(text=f"Erro na previsão: {str(e)}")
