...
class DrawPredict:
    def __init__(self):
        ...
        self.class_names = ["avião", "abelha", "bicicleta", "gato", "cachorro",
                            "flor", "casa", "carinha feliz", "caracol", "árvore"]
        self.load_model()
        self.init_train_GUI()

        self.prob_label = customtkinter.CTkLabel(self.root, text="Probabilidades:", justify="left")
    ...
    def init_train_GUI(self):
        ...
        try:
            ...
            messagebox.showerror("Erro", "Uma ou mais imagens de botão não foram encontradas.")
            self.root.destroy()
            return
    ...
    def predict_proba(self):
        ...
        img = self.image.convert("L").resize((28, 28), PIL.Image.LANCZOS)
        img_array = np.array(img).reshape(1, -1) / 255.0
        probs = self.classifier.predict_proba(img_array)[0]
        return probs
    ...
    def predict(self):
        try:
            ...
            return f"Desenho reconhecido como: {self.class_names[index]}"
    ...
    def update_prediction_loop(self):
        try:
            ...
            self.prob_label.configure(text="Probabilidades: (modelo não treinado)")
